...
import { Mail, Lock, User as UserIcon, Eye, EyeOff, Rocket } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      const user: User = {
        ...
        name: formData.name || formData.email.split('@')[0].charAt(0).toUpperCase() + formData.email.split('@')[0].slice(1)
      };
      onRegister(user);
      setIsLoading(false);
    }, 1500);
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="glass-effect rounded-2xl p-8 cosmic-glow">
          <form className="space-y-6" onSubmit={handleSubmit}>
            {/* Name Field */}
            <div>
              <label htmlFor="name" className="block text-caption font-semibold text-cosmic-200 mb-3">
                Full Name
              </label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                  <UserIcon className="h-5 w-5 text-cosmic-400" />
                </div>
                <input
                  id="name"
                  name="name"
                  ...
                  className={`block w-full pl-12 pr-4 py-4 border rounded-xl bg-cosmic-800/50 text-white placeholder-cosmic-400 focus:ring-2 focus:ring-cosmic-500 focus:border-transparent transition-all duration-300 text-body ${
                    errors.name ? 'border-red-500' : 'border-cosmic-600'
                  }`}
            ...
            <div className="relative my-6">
              ...
              <div className="relative flex justify-center text-caption">
                <span className="bg-cosmic-900 px-4 text-cosmic-400">Already have an account?</span>
              </div>
            </div>
          ...
          <div className="mt-6 p-4 bg-cosmic-800/30 rounded-xl border border-cosmic-700/50">
            <p className="text-xs text-cosmic-400 text-center mb-2">
              <strong>Demo Mode:</strong> This is a demonstration platform
            </p>
            <div className="text-xs text-cosmic-500 text-center">
              <p>No real account will be created. Use any valid information to explore.</p>
            </div>
          </div>
        </div>
      </div>
